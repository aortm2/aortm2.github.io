...
jQuery(function($){
	...
	$(".sec_01").resizable({
		...
		resize: function (e, ui) {
			...
			$(".sec_01").css("width",  currentWidth)
			$(".sec_02").css("width", "calc(100% - " + currentWidth + "px)");
		}
	});
	...
	$('.sheet .p_close').on('click', function(e){
		e.preventDefault();
		var popup = $(this).parents('.sheet');
		//console.log(popup)
		if(popup.length > 0){
			popup.fadeOut();
			aswBtn.removeClass("on");
			$(".sheet").hide()
		}
	});
	aswBtn.click(function(){
		if(!$(this).hasClass("on")){
			$(this).addClass("on");
			$(".sheet").show()
		}else{
	...
	// Directions
	var dirBtn = $(".b_dir")
	$('.dir .dir_close').on('click', function(e){
		var popup = $(this).parents('.dir');
		//console.log(popup)
		if(popup.length > 0){
			popup.fadeOut();
	...
	//b_size
	//20230316 : 전체에 비율로 토글되도록 수정
	(function(){
		var sbRight = $(".b_size.right"),
			sbLeft = $(".b_size.left"),
			sec01 = $(".sec_01"),
			sec02 = $(".sec_02");
		sbRight.on('click', function(e){
			e.preventDefault();
			sat_half = $(".sat_con").width()/2;
			if(sec01.outerWidth() < sat_half || sec01.outerWidth() > sat_half){
				sec01.css("width","50%");
				sec02.css("width","50%");
			}else if(sec01.outerWidth() == sat_half){
				$(".section").removeAttr('style');
				sec01.css("width","75%");
				sec02.css("width","25%");
			}
		});
		sbLeft.on('click', function(e){
			e.preventDefault();
			sat_half = $(".sat_con").width()/2;
			if(sec02.outerWidth() < sat_half || sec02.outerWidth() > sat_half){
				sec02.css("width","50%");
				sec01.css("width","50%");
			}else if(sec01.outerWidth() == sat_half){
				$(".section").removeAttr('style');
	...
	// 	if(sec01.width() >= 960){
	// 		sec01.css("width","956px")
	// 	}else{
	// 		$(".section").removeAttr('style')
	// 		sec01.css("width","1500px")
	// 	}
	// 	if(sec01.width() >= 960){
	// 		sec01.css("width","956px")
	// 	}else{
	...
	// 	}
	// });
	// var sbLeft = $(".b_size.left")
	// 	sec02 = $(".sec_02")
	// sbLeft.on('click', function(){
	// 	if(sec02.width() >= 960){
	// 		sec02.css("width","960px")
	// 	}else{
	// 		$(".section").removeAttr('style')
	// 		sec02.css("width","1500px")
	// 	}
	// });
	
	// undo
	$(".b_check").click(function(){
		if(!$(".example").hasClass("on")){
			$(".example").addClass("on");
			$(this).addClass("on")
		}else{
			$(".example").removeClass("on");
			$(this).removeClass("on")
			$(".example li").removeClass("ud")
		}
	});
	$(".example li > div").on('click',function(){
		exli = $(this).parents("li")
		if(!exli.hasClass("on")){
			$(".example li").removeClass("on");
			exli.addClass("on");
		}else{
			exli.removeClass("on");
		}
	...
	$('.highlight .p_close').on('click', function(e){
		e.preventDefault();
		var popup = $(this).parents('.highlight');
		//console.log(popup)
		if(popup.length > 0){
			popup.fadeOut();
		}
	...
	// 수학 주관식
	$(".an_input").keyup(function(){
		var answer = $(this).val()
		$(".an_pr").val(answer)
	});

	// reference pop
	$(".b_winsize").on('click', function(){
		var dialog = $(this).parents(".c_dialog");
